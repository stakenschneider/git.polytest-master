...
DECLARE_KHOOK(sys_fork);
int khook_sys_fork(struct pt_regs *regs)
{
    int result;

    KHOOK_USAGE_INC(sys_fork);

    printk("System call for fork hooked\n");

    result = KHOOK_ORIGIN(sys_fork, regs);

    KHOOK_USAGE_DEC(sys_fork);
