...
A = [1,1,5,5;
    ...
    3,3,12,12;
    0,-45,0,-21];
b = [Tmax1; -Tmin1; -Tmin2;  5000; 9000; 1950; -1000];
Aeq = [];
beq = [];
...
[Ndet, ~] = fmincon(f, startingPoint,[2,2,7,7; A], [Tmax1; b], Aeq, beq, lb, ub, [], options);
% Решение стохастической задачи:
N = zeros(4,numel(alpha));
for i = 1:numel(alpha)
 K = Ka(i);
 [N(:,i), ~] = fmincon(f, startingPoint,  A, b, Aeq, beq, lb, ub, @nonlin, options);
end
%% Вывод результатов
